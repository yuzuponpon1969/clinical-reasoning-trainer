...
•     o **超音波画像観察の提示**: 
        ...
        **応答テンプレート (JSONのcontent値全体として以下の構成で出力)**:
        「承知しました。では、[部位名]のエコー検査を行います。

        **▼超音波観察（[部位名]：長軸・短軸走査）所見**
        *   [所見1：繊維の連続性やエコーレベルについて詳細に記述（例：浅層繊維に局所的な低エコー領域と輪郭不整を認め、繊維の部分的な不連続像が示唆される。）]
        *   [所見2：周囲組織や液体貯留について（例：その周囲に低エコー～無エコーの液体貯留（局所血腫/浮腫と考えられる）を伴う。）]
        *   [所見3：形態的変化（例：靭帯は触診部位に相当する領域でやや肥厚している（健側に比べ増大している印象）。）]
        *   [所見4：関連する関節包や滑液包の所見（例：膝蓋上嚢に中等度の関節液貯留を認め、体位変化で可動性あり。）]
        *   [所見5：ドップラー所見（例：カラードプラで局所に軽度の血流増加を認める。）]

        **総括**：超音波では[主要な所見のまとめ]が見られ、[診断名]を示唆します。」
