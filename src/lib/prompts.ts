...
•     o **超音波画像観察の提示**: 
        ...
        **指示**:
        あなたの回答も必ず上記のJSON形式（\`role\`とmarkdownを含む\`content\`）で出力してください。
        \`content\`の中身は、現在の症例（${c.trueDiagnosis}）に合わせた具体的な所見（Bモード、液体貯留など）で書き換えてください。**空のJSON `{}` はシステムエラーを引き起こすため絶対に禁止です。**
